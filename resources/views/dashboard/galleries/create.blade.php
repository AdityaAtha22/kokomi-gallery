...
<div class="overflow-y-scroll text-dark fontInter bg-white">
  <div class="container">
    {{-- page title --}}
  <h3 class="text-3xl lg:text-4xl font-medium mt-6 mb-8">Tambah Data</h3>
  ...
  <form method="post" action="/dashboard/galleries" enctype="multipart/form-data">
    ...
    <div class="mb-5">
      ...
      <input class="w-full lg:w-96 rounded-md border px-3 py-2 border-slate-400 @error('title') border-red-500 @enderror" type="text" placeholder="Masukan title" value="{{ old('title') }}" name="title" id="title" autofocus autocomplete="off" />
      {{-- jika input error --}}
      @error('title')
    ...
    <div class="mb-5">
      ...
      <input class="w-full lg:w-96 rounded-md border px-3 py-2 border-slate-400 @error('slug') border-red-500 @enderror" type="text" placeholder="Tidak perlu diisi" value="{{ old('slug') }}" name="slug" id="slug" autofocus autocomplete="off" readonly />
      {{-- jika input error --}}
      @error('slug')
      <div class="text-red-500">{{ $message }}</div>
      @enderror
    ...
    <div class="mb-5 w-full lg:w-1/2">
      ...
      <input class="flex flex-wrap" id="body" value="{{ old('body') }}" type="hidden" name="body">
    ...
    {{-- tombol tambah data --}}
    <button type="submit" class="w-full lg:w-auto inline-block text-white bg-green-600 hover:bg-green-700 rounded-md hover:drop-shadow-md my-4 px-10 py-2 transition duration-100 ease-in">Create</button>
  </form>
  </div>
</div>
@endsection